        ...
        public override bool OnDraw()
        {
            ...
            for (int x = 0; x < ScreenWidth; ++x)
            {
                ...
                for (int y = 0; y < ScreenHeight; ++y)
                {
                    ...
                    NECharacterCell sa = NECharacterCell.MakeColFromBlocks5(0, 8, luma * 0.5f);
                    NEScreenBuffer.PutChar(sa.Character, sa.BitMask, x, y);
                    if (yNormRev > m_SignalBands[currentBand]) continue;
                    if (x % 8 == 0) continue;
                    NECharacterCell cs = NECharacterCell.MakeColFromBlocks5(4, 12, (1.0f - NEMath.Pow(yNorm, 1.987f))*(luma));
